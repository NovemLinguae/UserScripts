...
mw.hook('wikipage.content').add(function($content) {
				...
				.append($('<button style="margin:4px;">Save</button>').click(function() {
					...
					$.get(mw.config.get('wgScript'), {
						...
						$.post(mw.config.get('wgScriptPath') + '/api.php', {
							...
							summary: ((type.charAt(0).toUpperCase() + type.slice(1)) + 'ed').replace('ee', 'e').replace('Commented', 'Commented on') + ' request using [[en:w:User:BrandonXLF/FFUHelper|FFU Helper]]'
						}).done(function() {
	...
	$content.find('.mw-parser-output h2').each(function() {
		let $el = $('[href*="title="][href*="section="]', this);
		let href = $el.attr('href');
		if ( href === undefined ) {
			return;
		}
		var section = /[?&]v?e?section=T?-?(\d*)/.exec(href)[1];
