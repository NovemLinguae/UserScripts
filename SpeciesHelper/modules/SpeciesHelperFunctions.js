...
export class SpeciesHelperFunctions {
	...
	async doPagesExist( listOfPages ) {
		const api = new mw.Api();
		const response = await api.get( {
			action: 'query',
			format: 'json',
	...
	goToShowChangesScreen( titleWithNamespaceAndUnderscores, wikicode, editSummaryItems ) {
	...
	getPagesToCheck( taxa, listOfNonLatinSpeciesCategories ) {
		...
		for ( let piece of chunk ) {
			...
			pagesToCheck = this.deleteFromArray( 'Template:Viola-stub', pagesToCheck );
		}
	...
	parseListOfPages( listOfPages, type ) {
	...
	getAllTaxaCategories( listOfPages ) {
	...
	fixArrayOrder( correctOrder, incorrectOrder ) {
	...
	isMinorChange( wikicode, wikicode2 ) {
	...
	arraysHaveSameValuesCaseInsensitive( array1, array2 ) {
	...
	deleteAllStubs( wikicode ) {
		return wikicode.replace( /\{\{[^}]*-stub\}\}\n/gi, '' );
	}

	isSandbox( titleWithNamespaceAndUnderscores ) {
		return !!titleWithNamespaceAndUnderscores.match( /sandbox/i );
	}

	async getPageCreationDate( title ) {
		const api = new mw.Api();
		const response = await api.get( {
			action: 'query',
			format: 'json',
	...
	fixSpeciesParameterThatContainsGenus( wikicode2 ) {
		const hasSpeciesBox = this.getSpeciesBox( wikicode2 );
	...
	getSpeciesBox( wikicode2 ) {
		return wikicode2.match( /\{\{(?:Speciesbox|Species[ _]box)/i );
	}

	removeItalicTitleIfSpeciesBoxPresent( wikicode2 ) {
		const hasSpeciesBox = this.getSpeciesBox( wikicode2 );
		if ( hasSpeciesBox ) {
			// remove {{Italic title}}
	...
	replaceReferencesWithReflist( wikicode2 ) {
	...
	getListOfCategoriesFromWikitext( wikicode2 ) {
		const allCategoriesRegEx = /(?<=\[\[:?)Category:.+?(?=\]|\|)/gi;
		return wikicode2.match( allCategoriesRegEx );
	}

	suggestShortDescriptionFromWikicode( wikicode2, disallowedList = [] ) {
		...
		for ( const disallowed of disallowedList ) {
			const regEx = new RegExp( this.regExEscape( disallowed ), 'i' );
		...
		let lastWord = this.getLastWord( wikicode2 );
	...
	deleteMoreThanTwoEntersInARowBeforeReferences( wikicode, mopf ) {
	...
	removeEmptyDefaultSort( wikicode2 ) {
		wikicode2 = wikicode2.replace( /\{\{(?:DEFAULTSORT|Default[ _]sort|SORTIERUNG)[:|]?\s*\}\}\n?/gi, '' );
		return wikicode2;
	}

	removeDefaultSortIdenticalToTitle( wikicode2, titleNoNamespaceNoUnderscores ) {
		const titleRegExEscaped = this.regExEscape( titleNoNamespaceNoUnderscores );
	...
	enableCategories( wikicode2, isDraft ) {
		if ( !isDraft ) {
	...
	disableCategories( wikicode2, isDraft ) {
		if ( isDraft ) {
			wikicode2 = wikicode2.replace( /\[\[Category:/gi, '[[:Category:' );
		}
		return wikicode2;
	...
	deleteGenusCategoryWithSpaceDisambiguator( wikicode2, genus, draftCategoryColon ) {
		const regEx = new RegExp( `\\[\\[${ draftCategoryColon }Category:` + this.regExEscape( genus ) + '\\| \\]\\]\\n', 'i' );
		return wikicode2.replace( regEx, '' );
	}

	removeDraftTagIfNotDraftspace( wikicode2, isDraft ) {
		if ( isDraft ) {
			return wikicode2;
		}
	...
	isDisambiguationPage( wikicode2 ) {
		return Boolean( wikicode2.match( /disambiguation(?:\}\}|\|)/i ) );
	}

	isRedirectPage( wikicode2 ) {
		return Boolean( wikicode2.match( /^[\n ]*#REDIRECT \[\[/is ) );
	}

	deleteStubTemplateIfAnyOtherStubsPresent( wikicode2 ) {
	...
	getSpeciesboxTaxonAndParentParameters( wikicode2 ) {
