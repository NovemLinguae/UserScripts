...
mw.util.addPortletLink(
	...
	'Run SpeciesHelper',
	'SpeciesHelper',
	'[Alt+A]'	// can't put comma here, silent error
);
...
async function speciesHelper(title) {
	...
	draftCategoryColon = '';
	let isDraft = mw.config.get('wgNamespaceNumber') === 118;
	let hasDraftCategoriesTemplate = wikicode2.match(/\{\{Draft[ _]categories/gi);
	...
	let hasVirusBox = wikicode2.match(/\{\{(?:Virusbox)/i);
	if ( hasVirusBox ) {
		alert(`No changes needed. (Viruses currently skipped)`);
		return;
	}

	let editSummaryItems = [];

	// CONVERT TAXOBOX TO SPECIESBOX ==========================================
	let wrongBox = wikicode2.match(/\{\{(?:Automatic[ _]taxobox|Taxobox)/i);
	if ( wrongBox ) {
		/*
		...
		*/
		...
		let isGastropod = wikicode2.match(/Gastropoda/i);

		if ( ! isGastropod ) {
			let ctsb = new ConvertToSpeciesBox();
	...
	let hasSpeciesboxTaxonAndParentParameters = getSpeciesboxTaxonAndParentParameters(wikicode2);
	...
	if ( ! taxa)  {
		if ( isSandbox(title) ) {
			// get bolded title
			let match = wikicode.match(/'{3,}(.*?)'{3,}/)[1];
			titleNoNamespace = match.replace(' ', '_');
		}
		let matches = titleNoNamespace.match(/^([^_]+)_([^_]+)/);
		if ( matches ) {
			taxonomyTemplateGenus = matches[1];
			genusForAlert = genusForAlert ? genusForAlert : taxonomyTemplateGenus;
			species = matches[2];
			taxa = await getTaxa(taxonomyTemplateGenus);
		}
	}
	...
	if ( ! taxa ) {
		let userWantsToCreateTaxonomy = confirm(`Template:Taxonomy/${genusForAlert} does not exist. Is this actually a species? If so, need to create Template:Taxonomy/${genusForAlert}. Press OK to create the template, or Cancel to go back to the article.`);
	...
	let displayGenus = taxonomyTemplateGenus.replace(/ \([^\)]+\)/, '');

	taxa = taxaStringToArray(taxa);

	let mopf = new MOSOrderPositionFinder(wikicode);

	// SPECIESBOX ================================================================
	let hasSpeciesBoxOrTaxoBox = wikicode2.match(/\{\{(?:Speciesbox|Species[ _]box|Automatic[ _]taxobox|Taxobox|Subspeciesbox|Infraspeciesbox|Hybridbox|Virusbox)/i);
...
| species = ${species}
	...
	if ( ! hasTaxonBar ) {
		let wikidataID = await getWikidataID(`${displayGenus} ${species}`);
		if ( wikidataID ) {
			let toAdd = `{{Taxonbar|from=${wikidataID}}}`;
	...
	let pagesToCheck = getPagesToCheck(taxa, listOfNonLatinSpeciesCategories);
	...
	let genusCategoryToCheck = listOfNonLatinSpeciesCategories[taxonomyTemplateGenus] ? listOfNonLatinSpeciesCategories[taxonomyTemplateGenus] : taxonomyTemplateGenus;
	wikicode2 = deleteGenusCategoryWithSpaceDisambiguator(wikicode2, genusCategoryToCheck, draftCategoryColon);

	// CATEGORY ================================================================
	let suggestedCategory = parseListOfPages(listOfPages, 'category');
	let categoryGenusRegEx = new RegExp(`\\[\\[${draftCategoryColon}Category:` + regExEscape(genusCategoryToCheck), 'i');
	let hasGenusParameterCategory = wikicode2.match(categoryGenusRegEx); // so basically, don't run if genus category is already present. importantly, includes genuses with parentheses, e.g. [[Category:Saara (lizard)]]
	if ( suggestedCategory && ! hasGenusParameterCategory ) {
		let wikicodeBeforeCategoryChanges = wikicode2;

		// build list of categories currently in the article
		let categoriesInArticle = getListOfCategoriesFromWikitext(wikicode2);
		...
		for ( let key in listOfNonLatinSpeciesCategories ) {
			let value = listOfNonLatinSpeciesCategories[key];
		...
		for ( let cat of categoriesToDelete ) {
			let regEx = new RegExp('\\[\\[:?' + regExEscape(cat) + '(?:\\|[^\\]]+)?\\]\\] {0,}\\n', 'gi');
			wikicode2 = wikicode2.replace(regEx, '');
		}

		let toAdd = ``;
		let suggestedCategoryNoParentheses = suggestedCategory.replace(/ \([A-Za-z]+\)/, '');
		...
		let categoriesInArticle2 = getListOfCategoriesFromWikitext(wikicode2);
		let categoryListsAreIdentical = arraysHaveSameValuesCaseInsensitive(categoriesInArticle, categoriesInArticle2);
	...
	// IMPROVE CATEGORIES ======================================================
	let hasCategories = wikicode2.match(/\[\[:?Category:/gi);
	let categoryCount = hasCategories !== null ? hasCategories.length : 0;
	let hasImproveCategories = wikicode2.match(/\{\{Improve[ _]categories/i);
	...
	// STUB ==================================================================
	let suggestedStubName = parseListOfPages(listOfPages, 'stub');
	let shouldBeStub = countWords(wikicode2) < 150; // I've been reverted for stub tagging an article with a word count of 175 before. so setting this kind of low.
	let hasStubTags = wikicode2.match(/\{\{.+\-stub\}\}\n/gi);
	// these stubs are kind of fuzzy for various reasons. not worth messing with them
	let stubsThatTriggerSkip = [
		'Iguanidae',
		'Lizard',
		'Potato',
	];
	...
	for ( let stub of stubsThatTriggerSkip ) {
		let regex = new RegExp('\\{\\{' + stub + '-stub\\}\\}', 'i');
		let hasStub = wikicode2.match(regex);
		if ( hasStub ) {
			skip = true;
			break;
		}
	...
	if (
		...
		let oldStubs = [];
		let matches = wikicode2.match(/\{\{[^}]+-stub\}\}/gi);
		for ( key in matches ) {
	...
	let allCategoriesRegEx = new RegExp(`(?<=\\[\\[${draftCategoryColon})Category:.+(?=\\]\\])`, 'gi');
	...
	let hasShortDescription = wikicode2.match(/\{\{(?:Short[ _]description|Shortdesc|Shortdescription|Short desc)/i);
	if ( ! hasShortDescription ) {
		let suggestedShortDescription = suggestShortDescriptionFromWikicode(wikicode2, taxa);
	...
	*/
	...
	wikicodeBefore = wikicode2;
	let titleNoNamespaceNoUnderscores = titleNoNamespace.replace('_', ' ');
	...
	let editSummary = '';
	for ( let item of editSummaryItems ) {
		editSummary += `${item}, `;
	}
