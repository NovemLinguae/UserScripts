...
$(function() {
	class NPPLinks {
		...
		_checkForForeignCharacters() {
			...
			let regEx = new RegExp(`^[${this.listOfNonForeignCharacters}]*$`, '');
			let ansiOnly = regEx.test(this.articleBody); // the ones at the end are vietnamese
			if ( ! ansiOnly ) {
		...
		_debugForeignCharacterDetection() {
			...
			let regEx = new RegExp(`[^${this.listOfNonForeignCharacters}]`, 'g');
			let matches = this.articleBody.match(regEx);
			if ( matches ) {
