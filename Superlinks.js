        ...
        var app = {
            ...
            init: function () {
                function mk(caption, group) {
                    var $link = $("<a>", {'href': '#'}).text(caption);
                    if (group) group.append($link)
                    else $("#superlinks-links").append($link);
                ...
                if (app.relevantUser) app.relevantUser = mw.util.wikiUrlencode(app.relevantUser);
            ...
            click: function(e) {
                function msg(text) { //blank for loading icon
                    ...
                    else $div.append($("<img>", {'src': "https://upload.wikimedia.org/wikipedia/commons/3/30/Chromiumthrobber.svg"}));
                ...
                function afterLoad() {
                    if (app.$content.children().length===0) msg("No results");
                                ...
                                if (data.query.abuselog.filter(item => (item.revid)).length) {
                                    $.each(data.query.abuselog, function (i, item) {
                                        if (item.result=='tag' && item.revid) {
                                            var $item = $("<div>").appendTo(app.$content);                                        
                                            $.getJSON('/w/api.php', {
                                                action: 'compare', 
                                                format: 'json', 
                                                fromrev: item.revid, 
                                            ...
                                            .done(function(comparedata) {
                                                ...
                                                var ts = new Date(item.timestamp).toUTCString();
                                                ...
                                                var diff = '';
                                                $(comparedata.compare["*"]).find("td.diff-addedline > div").each(function (i,item) {
                                                    diff += $(item).html() + "\n";
                                                });
...
//                                '&prefix=Wikipedia%3AArbitration+enforcement+log%2F&title=Special%3ASearch&profile=advanced&fulltext=1&advancedSearch-current=%7B%7D&ns4=1');
                    ...
                    app.keyup = $("body").on("keyup", function(event) {
                        if (event.which == 27) {
                            app.close();
                        }
                    });
